  ...
  while(1) {
    ...
    // ####### FEEDBACK SERIAL OUT #######
      ...
      if (main_loop_counter % 2 == 0) {    // Send data periodically every 10 ms
        ...
        Feedback.speedL_meas	  = -(int16_t)rtY_Left.n_mot; // one wheel spins backwards, now signs match torque request
        ...
        Feedback.currR_meas     = (int16_t)right_dc_curr;
          ...
          if(__HAL_DMA_GET_COUNTER(huart2.hdmatx) == 0) {
            ...
            Feedback.checksum   = (uint16_t)(Feedback.start ^ Feedback.cmd1 ^ Feedback.cmd2 ^ Feedback.speedR_meas ^ Feedback.speedL_meas 
                                           ^ Feedback.batVoltage ^ Feedback.boardTemp ^ Feedback.currL_meas ^ Feedback.currR_meas);
